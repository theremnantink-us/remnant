...
(function initSmoke() {
  ...
  // Мягкие светло-серые облака — видны на тёмном фоне, не мешают контенту
  const COLORS = [
    { r: 155, g: 158, b: 162 },
    { r: 130, g: 133, b: 138 },
    { r: 170, g: 172, b: 175 },
  ...
  function createParticle(init) {
    ...
    const size = (280 + Math.random() * 520) * SCALE;
    return {
      x: init ? Math.random() * W : -size * 0.5,
      y: H * 0.05 + Math.random() * H * 0.95,
      size, vx: (0.10 + Math.random() * 0.22) * SCALE,
      vy: (-0.06 - Math.random() * 0.14) * SCALE,
      opacity: 0.06 + Math.random() * 0.08, color,
      phase: Math.random() * Math.PI * 2,
      waveAmp: 7 + Math.random() * 18, waveSpeed: 0.14 + Math.random() * 0.30,
      life: init ? Math.random() : 0, growSpeed: 0.0004 + Math.random() * 0.0009,
    };
  }

  for (let i = 0; i < PARTICLE_COUNT; i++) particles.push(createParticle(true));

  // Облака видны по всей странице кроме подвала
  let smokeOpacity = 1;
  const footer = document.querySelector('.footer');
  window.addEventListener('scroll', () => {
    if (footer) {
      const footerTop = footer.getBoundingClientRect().top;
      // Начинаем скрывать только когда футер входит в viewport
      if (footerTop < window.innerHeight) {
        smokeOpacity = Math.max(0, footerTop / window.innerHeight);
      } else {
        smokeOpacity = 1;
      }
    }
  ...
  function drawSmoke(p, pulsedSize) {
    const radius = pulsedSize * 0.5;
    const gradient = ctx.createRadialGradient(p.x, p.y, 0, p.x, p.y, radius);
    const { r, g, b } = p.color;
    const a = p.opacity * p.life * smokeOpacity;
    gradient.addColorStop(0,   `rgba(${r},${g},${b},${a})`);
    gradient.addColorStop(0.2, `rgba(${r},${g},${b},${a * 0.8})`);
    gradient.addColorStop(0.5, `rgba(${r},${g},${b},${a * 0.35})`);
    gradient.addColorStop(0.8, `rgba(${r},${g},${b},${a * 0.08})`);
  ...
  function animateSmoke(now) {
    ...
    if (smokeOpacity < 0.01) return;

    time += 0.016;
    for (let i = 0; i < particles.length; i++) {
      const p = particles[i];
      if (p.life < 1) p.life = Math.min(1, p.life + p.growSpeed * 60);
      p.x += p.vx;
      p.y += p.vy + Math.sin(time * p.waveSpeed + p.phase) * 0.3 * SCALE;
      const pulsedSize = p.size * (1 + Math.sin(time * 0.3 + p.phase) * 0.08);
      drawSmoke(p, pulsedSize);

      if (p.x > W + p.size || p.y < -p.size) {
        particles[i] = createParticle(false);
        if (Math.random() > 0.3) {
          particles[i].x = -particles[i].size * 0.5;
          particles[i].y = H * 0.2 + Math.random() * H * 0.8;
        } else {
          particles[i].x = Math.random() * W;
          particles[i].y = H + particles[i].size * 0.3;
        }
      }
    }
  }
